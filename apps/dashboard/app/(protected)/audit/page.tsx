      ...
      <DashboardCard
        ...
        actions={
          <div className="flex items-center gap-2 text-xs text-ink-muted">
            <Clock size={12} />
            {logsQuery.data?.total ?? 0} {t('entries')}
          </div>
        }
      >
        {/* Filters */}
        <div className="mb-5 flex flex-wrap items-center gap-3">
          ...
          >
            <option value="" className="bg-background text-ink">{t('All actions')}</option>
        ...
        {logsQuery.isLoading ? (
          ...
          <EmptyState
            ...
            description={t('Audit logs will appear as fleet actions are recorded.')}
          />
        ) : (
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-sm font-medium text-ink">
                        {t(formatEnumLabel(log.actionType))}
                      </p>
                      <Badge label={t(log.targetType)} tone="neutral" />
                    </div>
                    <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-ink-muted">
                      <span>{formatTimestamp(log.createdAt)}</span>
                      {log.actorUser?.email && (
                        <span>{t('by')} {log.actorUser.email}</span>
                      )}
                      {log.targetId && (
