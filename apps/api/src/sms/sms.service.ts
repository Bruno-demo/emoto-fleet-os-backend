...
export class SmsService {
  ...
  constructor(
    ...
    this.enabled = this.configService.get<boolean>('SMS_FALLBACK_ENABLED', true);
  ...
  public async sendSms(to: string, message: string): Promise<SendSmsResult> {
    if (!this.enabled) {
      this.logger.debug(`SMS dispatch disabled by configuration. Message to ${to}: "${message}"`);
      return { success: false, provider: 'disabled', error: 'SMS fallback disabled by config' };
    }
    ...
    switch (this.provider) {
      ...
      case 'log':
      default:
        this.logger.log(`[SMS FALLBACK LOG] To: ${recipient} | Message: "${message}"`);
   ...
   */
  private async sendViaAfricasTalking(to: string, message: string): Promise<SendSmsResult> {
    ...
    if (!apiKey) {
      this.logger.warn('AFRICASTALKING_API_KEY is not configured. Falling back to log.');
      this.logger.log(`[SMS FALLBACK LOG] To: ${to} | Message: "${message}"`);
      return { success: true, provider: 'log-fallback', messageId: `log-${Date.now()}` };
    }
    ...
    try {
      const response = await firstValueFrom(
        this.httpService.post(baseUrl, params.toString(), {
          headers: {
            apiKey,
      ...
      const status = recipientData?.status || JSON.stringify(response.data);
      ...
      this.logger.error(`Failed to send SMS via Africa's Talking to ${to}: ${errorMsg}`);
   ...
   */
  private async sendViaTwilio(to: string, message: string): Promise<SendSmsResult> {
   ...
   */
  private async sendViaGenericWebhook(to: string, message: string): Promise<SendSmsResult> {
    const webhookUrl = this.configService.get<string>('GENERIC_SMS_WEBHOOK_URL');
    const webhookToken = this.configService.get<string>('GENERIC_SMS_WEBHOOK_TOKEN');

    if (!webhookUrl) {
      this.logger.warn('Generic SMS Webhook URL not configured. Falling back to log.');
      this.logger.log(`[SMS FALLBACK LOG] To: ${to} | Message: "${message}"`);
      return { success: true, provider: 'log-fallback', messageId: `log-${Date.now()}` };
    }
    ...
    try {
      ...
      this.logger.log(`Dispatched SMS via generic webhook/Android gateway to ${to}: ${message}`);
      return {
        success: true,
        provider: 'generic',
        messageId: response.data?.messageId || response.data?.id || `generic-${Date.now()}`,
      };
    } catch (err: any) {
      const errorMsg = err.response?.data?.message || err.message;
      this.logger.error(`Failed to dispatch SMS via generic webhook to ${to}: ${errorMsg}`);
