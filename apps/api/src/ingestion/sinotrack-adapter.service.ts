...
export class SinoTrackAdapterService implements OnModuleInit, OnModuleDestroy {
  ...
  private async processTelemetryPacket(
    ...
    // Telemetry-driven command auto-acknowledgement (verifies physical relay cut/restore)
    try {
      if (mainPowerCut || !ignition) {
        const pendingLock = await this.prismaService.deviceCommand.findFirst({
          where: {
            deviceId: device.id,
            type: 'LOCK',
            status: { in: ['PENDING', 'SENT'] },
        ...
        if (pendingLock) {
          ...
          this.eventsGateway.emitCommandStatus(device.fleetId, {
            commandId: pendingLock.id,
            status: 'ACKED',
            ts: new Date().toISOString(),
            bikeId: device.bikeId ?? undefined,
            deviceId: device.id,
            action: 'LOCK',
            message: 'Vehicle power cut confirmed via live telemetry',
          });
        }
      } else if (ignition && !mainPowerCut) {
        const pendingUnlock = await this.prismaService.deviceCommand.findFirst({
          where: {
            deviceId: device.id,
            type: 'UNLOCK',
            status: { in: ['PENDING', 'SENT'] },
        ...
        if (pendingUnlock) {
          ...
          this.eventsGateway.emitCommandStatus(device.fleetId, {
            commandId: pendingUnlock.id,
            status: 'ACKED',
            ts: new Date().toISOString(),
            bikeId: device.bikeId ?? undefined,
            deviceId: device.id,
            action: 'UNLOCK',
            message: 'Vehicle power restore confirmed via live telemetry',
          });
        }
      }
    } catch (cmdAckErr: unknown) {
      const msg = cmdAckErr instanceof Error ? cmdAckErr.message : 'unknown';
      this.logger.debug(`Telemetry command confirmation skipped: ${msg}`);
    }
